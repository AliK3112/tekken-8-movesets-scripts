...
def encrypt_original_value(original_value: int) -> int:
    ...
    bit_offset = 0
    masked_value = (
        (original_value ^ (seed & 0xFFFFFFE0) ^ 0x1D) & 0xFFFFFFF
    ) & 0xFFFFFFFF
...
def validate_input(value: str) -> int:
    ...
    if not match:
        raise ValueError(
            "Invalid input format. Enter a 32-bit integer in decimal or hex format."
        )
...
def process_input():
    try:
        ...
        output_var.set(hex(encrypted_value).upper())
...
tk.Button(frame, text="Encrypt", command=process_input).grid(
