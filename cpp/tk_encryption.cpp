...
uint32_t TK__pack20BitWith12BitChecksum(uint32_t input, uint64_t key) {
  ...
  for (uint32_t byte_offset = 0; byte_offset < 20; byte_offset += 8) {
    ...
    uint64_t rotated_magic = key;
    uint8_t rotate_count = byte_offset + 8;

    while (rotate_count--) {
      rotated_magic =
          (rotated_magic >> 63) | (rotated_magic << 1); // Rotate left
    }
...
uint32_t TK__pack24BitWith8BitChecksum(uint32_t input, uint64_t key) {
  uint8_t checksum = 0;
...
uint32_t TK__pack26BitWith6BitChecksum(uint32_t input, uint64_t key) {
  uint8_t checksum = 0;
...
uint32_t TK__pack28BitWith4BitChecksum(uint32_t input, uint64_t key) {
  uint8_t checksum = 0;
...
uint64_t TK__pack32BitWith32BitChecksum(uint32_t input, uint64_t key) {
  uint32_t checksum = 0;
  ...
  for (uint32_t byte_offset = 0; byte_offset < 32; byte_offset += 8) {
    ...
    while (rotate_count--) {
      rotated_magic =
          (rotated_magic >> 63) | (rotated_magic << 1); // Standard rotate left
    }

    // XOR 32-bit remainder with 32-bit rotated magic (as per real
    // implementation)
...
uint64_t TK__pack56BitWith8BitChecksum(uint64_t input, uint64_t key) {
  uint8_t checksum = 0;
...
uint32_t TK__encrypt20BitWith12BitChecksum(uint32_t original_value,
  ...
  uint32_t baseEncrypted =
      (original_value ^ (rotatedKey & 0xFFFFFFE0) ^ 0x1D) & 0x000FFFFF;
  ...
  // Final 32-bit result: upper 12 bits = checksum, lower 20 bits = encrypted
  // data
...
uint32_t TK__encrypt26BitWith6BitChecksum(uint32_t original_value,
  ...
  uint32_t baseEncrypted =
      (original_value ^ (rotatedKey & 0xFFFFFFE0) ^ 0x1D) & 0x03FFFFFF;
  uint32_t temp = baseEncrypted;
  uint8_t checksum = 0;
...
uint32_t TK__encrypt28BitWith4BitChecksum(uint32_t original_value,
  ...
  uint32_t baseEncrypted =
      (original_value ^ (rotatedKey & 0xFFFFFFE0) ^ 0x1D) & 0x0FFFFFFF;
  uint32_t temp = baseEncrypted;
  uint8_t checksum = 0;
  ...
  // Final 32-bit value: upper 4 bits = checksum, lower 28 bits = encrypted
  // value
...
uint64_t TK__encrypt56BitWith8BitChecksum(uint64_t original_value,
  ...
  uint64_t baseEncrypted =
      (original_value ^ (rotatedKey & 0xFFFFFFFFFFFFFFE0ull) ^ 0x1D) &
...
uint32_t TK__decrypt20BitWith12BitChecksum(uint32_t value, uint64_t key) {
  // 1. Validate checksum first
  if (TK__pack20BitWith12BitChecksum(value, key) != value) {
    return 0; // Invalid checksum
  }

  // 2. Begin transformation process
  uint32_t transformed = value ^ 0x1D;
  uint64_t scrambled = key;
...
uint32_t TK__decrypt24BitWith8BitChecksum(uint32_t value, uint64_t key) {
  // Validate checksum first (must match packed version)
  if (TK__pack24BitWith8BitChecksum(value, key) != value) {
    return 0; // Invalid checksum
  }
...
uint32_t TK__decrypt26BitWith6BitChecksum(uint32_t value, uint64_t key) {
  // Validate checksum
  if (TK__pack26BitWith6BitChecksum(value, key) != value)
    return 0;
...
uint32_t TK__decrypt28BitWith4BitChecksum(uint32_t value, uint64_t key) {
  // Validate checksum first
  if (TK__pack28BitWith4BitChecksum(value, key) != value) {
    return 0; // Invalid checksum
  }
...
uint64_t TK__decrypt32BitWith32BitChecksum(uint64_t value, uint64_t key) {
  // 1. Validate checksum
  if (TK__pack32BitWith32BitChecksum(value, key) != value) {
  ...
  // 4. Core transformation
  uint64_t intermediate = (transformed ^ (scrambled & 0xFFFFFFFFFFFFFFE0))
                          << 32;
  ...
  double result = static_cast<double>(intermediate) /
                  (high_bit + static_cast<uint64_t>(divisor));
  return static_cast<uint64_t>(result);
}

uint64_t TK__decrypt56BitWith8BitChecksum(uint64_t value, uint64_t key) {
  // 1. Validate checksum first
  if (TK__pack56BitWith8BitChecksum(value, key) != value) {
    return 0; // Invalid checksum
  }
